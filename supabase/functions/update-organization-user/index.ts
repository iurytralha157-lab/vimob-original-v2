...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let targetOrgId = requestedOrgId || actor.organization_id || target.organization_id;
    ...
    if (!requestedOrgId) {
      const { data: actorTargetMembership } = await supabaseAdmin
        .from('organization_members')
        .select('organization_id, role, is_active')
        .eq('user_id', actorId)
        .eq('organization_id', target.organization_id || actor.organization_id || '')
        .maybeSingle();

      const actorCanUseTargetCurrentOrg =
        target.organization_id &&
        (
          isSuperAdmin ||
          (actor.organization_id === target.organization_id && actor.role === 'admin') ||
          (actorTargetMembership?.role === 'admin' && actorTargetMembership?.is_active !== false)
        );

      if (actorCanUseTargetCurrentOrg) {
        targetOrgId = target.organization_id;
      } else {
        const { data: inferredMembership } = await supabaseAdmin
          .from('organization_members')
          .select('organization_id, role, is_active')
          .eq('user_id', userId)
          .limit(20);

        for (const membership of inferredMembership || []) {
          ...
          const { data: actorOrgMembership } = await supabaseAdmin
            .from('organization_members')
          ...
          if (actorOrgMembership?.role === 'admin' && actorOrgMembership?.is_active !== false) {
            targetOrgId = membership.organization_id;
            break;
          }
        }
    ...
    console.error('update-organization-user error:', errorMessage);
