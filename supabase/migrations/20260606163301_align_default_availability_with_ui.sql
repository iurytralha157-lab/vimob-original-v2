-- Keep backend distribution aligned with the availability dialog defaults.
-- When a member belongs to a team but has no saved availability rows yet, the
-- UI shows Monday-Friday 08:00-18:00. Distribution must follow the same default
-- instead of treating the member as 24h available.
...
BEGIN
  ...
  IF p_team_id IS NOT NULL THEN
    SELECT tm.id INTO team_member_id
    ...
    SELECT EXISTS (
      ...
      WHERE ma.team_member_id = team_member_id
        AND ma.is_active = true
    ) INTO v_has_availability_config;

    IF NOT v_has_availability_config THEN
      RETURN QUERY SELECT v_default_available, 'team_user_default_weekday_schedule'::text, team_member_id;
      RETURN;
    END IF;
