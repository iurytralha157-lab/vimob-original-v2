-- Respect member availability even when a round-robin member is configured as a
-- direct user instead of a team entry. Previously direct members were treated as
-- 24h available and could receive leads on inactive days.
...
BEGIN
  IF p_team_id IS NOT NULL THEN
    ...
    IF NOT v_has_availability_config THEN
      RETURN QUERY SELECT true, 'team_user_no_schedule_default'::text, team_member_id;
      RETURN;
    END IF;
  ...
  IF NOT v_has_availability_config THEN
    RETURN QUERY SELECT true, 'direct_user_no_schedule_default'::text, NULL::uuid;
    RETURN;
  END IF;
